...
for(const must of ['chatgpt_runtime','chatgpt_library','github','google_drive','google_docs','google_sheets','google_slides','google_apps_script','gemini_gems','hakim_control_plane','hakim_local_browser_bridge','make','unintegrated_external_platform']) if(!ids.has(must)) throw new Error('MISSING_PLATFORM:'+must);
...
if(!bridge || !['STATIC_CI_PASS_FIELD_PARTIAL','FIELD_SMOKE_PASS_SCOPE_LIMITED'].includes(bridge.mode)) throw new Error('HAKIM_BRIDGE_FIELD_BOUNDARY_MISSING');
if(bridge.mode==='FIELD_SMOKE_PASS_SCOPE_LIMITED' && bridge.field_evidence!=='hakim/FIELD_BRIDGE_EVIDENCE.json') throw new Error('HAKIM_BRIDGE_FIELD_EVIDENCE_MISSING');
