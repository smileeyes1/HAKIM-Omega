...
const phone=planMission('phone_web_ui'); eq(phone.status,'PASS'); eq(phone.route[0].id,'local_browser_agent'); eq(phone.route[0].cost_class,'VERIFIED_FREE');
const webFallback=planMission('web_ui_fallback'); eq(webFallback.status,'PASS'); eq(webFallback.route[0].id,'local_browser_agent'); ok(webFallback.blocked_by_cost.some(x=>x.id==='tinyfish_browser'),'TinyFish must be excluded by cost');
...
const onlyMetered=planMission('web_ui_fallback',['tinyfish_browser']); eq(onlyMetered.status,'BLOCKED'); eq(onlyMetered.reason,'BLOCKED_BY_COST'); eq(onlyMetered.route.length,0); ok(onlyMetered.blocked_tools.some(x=>x.id==='tinyfish_browser'));
...
const repo=planMission('repo_or_code'); eq(repo.status,'PASS'); eq(repo.route[0].id,'github_owner_relay'); eq(repo.route[0].cost_class,'VERIFIED_INCLUDED_IN_CURRENT_PLAN');
const google=planMission('google_native_files'); eq(google.status,'PASS'); eq(google.route[0].id,'chatgpt_reasoning'); ok(google.blocked_by_cost.some(x=>x.id==='google_control_plane'),'unknown-cost Google connector must be filtered');
const persist=planMission('persistent_artifact'); ok(persist.route.some(x=>x.id==='chatgpt_files_library')); ok(persist.route.some(x=>x.id==='github_owner_relay')); ok(persist.blocked_by_cost.some(x=>x.id==='google_control_plane'));

const costBlocked=structuredClone(cp);
costBlocked.tools.task_routes.cost_probe=['google_control_plane','tinyfish_browser'];
const blockedCost=planMission('cost_probe',null,costBlocked); eq(blockedCost.status,'BLOCKED'); eq(blockedCost.reason,'BLOCKED_BY_COST'); eq(blockedCost.blocked_tools.length,2);
...
eq(cp.tools.cost_guard.mode,'FAIL_CLOSED_FREE_ONLY_DEFAULT'); ok(cp.tools.cost_guard.autonomous_blocked.includes('COST_UNKNOWN')); ok(cp.tools.cost_guard.autonomous_blocked.includes('CREDIT_CONSUMING'));
for(const t of cp.tools.tools) ok(typeof t.cost_class==='string'&&t.cost_class.length>0,`missing cost class ${t.id}`);
...
console.log(JSON.stringify({pass:true,assertions:n,tool_count:v.tool_count,memory_layers:v.memory_layers,intent_states:v.intent_states,platform_count:v.platform_count,checked:['intent anti-drift','runtime authority boundary','fail-closed cost classes','free/included-only routing','credit and unknown-cost exclusion','GitHub included-plan evidence','native capability routes','fail-closed unavailable/unknown routes','memory layers','platform propagation boundaries','secret markers','baseline protection','field bridge scope-limited progression']},null,2));