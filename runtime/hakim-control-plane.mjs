...
export function validateControlPlane(cp = loadControlPlane()) {
  ...
  for (const inv of ['NO_FALSE_TOOL_OWNERSHIP','FAIL_CLOSED_ON_P0_OR_CRITICAL_UNKNOWN','INTENT_TO_OUTCOME_WITHOUT_GOAL_DRIFT','RUNTIME_AUTHORITY_DISCOVERY_REQUIRED','PROVEN_SUCCESS_FROZEN_BY_DEFAULT']) assert(manifest.protected_invariants.includes(inv), `protected invariant missing: ${inv}`);
  for (const phase of ['COMPILE_INTENT','DISCOVER_TOOLS','AUTHORITY_CHECK','VERIFY','REALITY_GATE','UPDATE_STATE']) assert(manifest.mission_state_machine.includes(phase), `mission state missing: ${phase}`);
  ...
  assert(['STATIC_CI_PASS_FIELD_PARTIAL','FIELD_SMOKE_PASS_SCOPE_LIMITED'].includes(pm.get('hakim_local_browser_bridge')?.mode), 'local bridge field boundary missing');
...
export function structureIntent(input = {}, cp = loadControlPlane()) {
  ...
  return {intent_id:input.intent_id||'RUNTIME_INTENT',statement,why:input.why??null,desired_outcome:desired,observable_acceptance:acceptance,hard_constraints:Array.isArray(input.hard_constraints)?input.hard_constraints:[],known_unknowns:unknowns,next_highest_safe_action:String(input.next_highest_safe_action||'').trim(),status:acceptance.length?(unknowns.length?'CLARIFIED':'LOCKED'):'CAPTURED',rule:'No hidden goal is invented; success requires observable acceptance.'};
}
...
export function status(cp = loadControlPlane()) {
  ...
  return {pass:v.pass,control_plane:cp.manifest.version,state:cp.state.status,release_state:cp.state.release_state,current_bottleneck:cp.state.mission.current_bottleneck,next_action:cp.state.mission.next_highest_safe_action,bridges:cp.state.bridge_health,tools_registered:v.tool_count,memory_layers:v.memory_layers,platform_registry:cp.platforms.registry_version};
}
...
function main(){
  ...
  try{
    ...
    else if(cmd==='plan'){const type=process.argv[3],out=planMission(type);console.log(JSON.stringify(out,null,2));if(!type||out.status!=='PASS')process.exitCode=2;}
    else if(cmd==='intent'){const raw=process.argv[3]?JSON.parse(process.argv[3]):{},out=structureIntent(raw);console.log(JSON.stringify(out,null,2));if(out.status==='BLOCKED')process.exitCode=2;}
    else throw new Error(`unknown command: ${cmd}`);
  }catch(err){console.error(JSON.stringify({pass:false,error:err.message},null,2));process.exitCode=1;}
}
