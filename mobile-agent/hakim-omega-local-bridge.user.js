...
(() => {
  ...
  const POLL_MS=20000, MAX_ACTIONS=50, MAX_WAIT=120000, MAX_DELAY=10000;
  ...
  function fetchMission(){return new Promise((resolve,reject)=>GM_xmlhttpRequest({method:'GET',url:MISSION_URL+'?t='+Date.now(),timeout:15000,headers:{'Cache-Control':'no-cache'},onload:r=>{try{if(r.status<200||r.status>=300)throw Error('MISSION_HTTP_'+r.status);resolve(JSON.parse(r.responseText))}catch(e){reject(e)}},onerror:()=>reject(Error('MISSION_NETWORK')),ontimeout:()=>reject(Error('MISSION_TIMEOUT'))}))}
